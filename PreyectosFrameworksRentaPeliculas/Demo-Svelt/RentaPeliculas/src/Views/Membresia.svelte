<script>
    let res = [] ;
	let activado = true;
    let MEMBRESIA={
                MEMID: '',
                CLIID: '',
                FECHAREGMEM: '',
                FECHAEXOMEM: '',
    };

    let cargarCombo = async () => {
        let url1 = "https://localhost:44392/api/Cliente/";
        let response1 = await fetch(url1);
        let Cliente = await response1.json();
        let est= Cliente;
        for (let i=0; i<est.length; i++) {
            var c = document.getElementById("comboCli");
            var option = document.createElement("option");
            option.value = est[i].CLIID;
            option.text = est[i].NOMBRECLI;
             c.appendChild(option);     
        }
	};

    
	let mostrarEmpresa = async () => {
        res =[];
		let url = 'https://localhost:44391/api/Membresia/';
        let response = await fetch(url);
        let membresias = await response.json();

        for(var i = 0; i < membresias.length; i++){
            let  modelo = {MEMID: '',NOMBRECLI: '', FECHAREGMEM: '',FECHAEXOMEM: ''};
            modelo.MEMID = membresias[i].MEMID;
            modelo.FECHAREGMEM = membresias[i].FECHAREGMEM;
            modelo.FECHAEXOMEM = membresias[i].FECHAEXOMEM;
            let url1 = "https://localhost:44392/api/Cliente/" + membresias[i].CLIID
            let response1 = await fetch(url1);
            let Cliente = await response1.json();
            modelo.NOMBRECLI = Cliente.NOMBRECLI;
            res.push(modelo);
        }
        res = res;
        console.log(res);
	};

	let agregarEmpreas = async () => {
        var e = document.getElementById("comboCli");
        var value = e.options[e.selectedIndex].value;

		const nuevoEmpreas = {
            MEMID: MEMBRESIA.MEMID,
            CLIID: value,
            FECHAREGMEM: MEMBRESIA.FECHAREGMEM,
            FECHAEXOMEM: MEMBRESIA.FECHAEXOMEM
		};

		fetch("https://localhost:44391/api/Membresia", {
			method: "POST",
			headers: { "Content-type": "application/json; charset=UTF-8" },
			body: JSON.stringify(nuevoEmpreas),
		})
			.then((respuesta) => respuesta.json())
			.then((datosRespuesta) => {
                mostrarEmpresa();
			})
			.catch(console.log);
	};

	let borrarEmpresa = (id) => {
		fetch("https://localhost:44391/api/Membresia/" + id, {
			method: "DELETE",
		})
			.then((respuesta) => respuesta.json())
			.then((datosRespuesta) => {
				mostrarEmpresa();
			})
			.catch(console.log);
	};

	let editarEmpresa = async (res) => {
        let url1 = "https://localhost:44391/api/Membresia/" + res.MEMID;
        let response1 = await fetch(url1);
        let Membresia = await response1.json();
        document.getElementById("comboCli").value = Membresia.CLIID;
        MEMBRESIA = res;
		activado=false;
	};

	let actualizarEmpresa = () => {
		fetch("https://localhost:44391/api/Membresia/" + MEMBRESIA.MEMID, {
			method: "PUT",
			headers: { "Content-type": "application/json; charset=UTF-8" },
			body: JSON.stringify(MEMBRESIA),
		})
			.then((respuesta) => respuesta.json())
			.then((datosRespuesta) => {
				console.log(MEMBRESIA);
				console.log(datosRespuesta);
				mostrarEmpresa();
			})
			.catch(console.log);
	};


	mostrarEmpresa();
    cargarCombo();
</script>

<body >
    <div class="pater">
        <div class="card border-dark" style="width: 30%; float:left"> 
                <div class="card-header">Membresías</div>
                <div class="card-body">
                    <form>
                        <div class="formgroup" >
                            <label for="" class="form-label">ID</label>
                            <input
                                type="text"
                                bind:value={MEMBRESIA.MEMID}
                                class="form-control"
                            />

                            <label for="" class="form-label">Cliente</label>
                            <select class="form-control" id="comboCli"></select>
                            
                            <label for="" class="form-label">Fecha Registro</label>
                            <input
                                type="text"
                                bind:value={MEMBRESIA.FECHAREGMEM}
                                class="form-control"
                            />
                            <label for="email" class="form-label">Fecha Expiración</label>
                            <input
                                type="text"
                                bind:value={MEMBRESIA.FECHAEXOMEM}
                                class="form-control"
                            />
                        </div>
                        <button
                            type="button"
                            class="btn btn-success"
                            on:click|preventDefault={agregarEmpreas}
                            
                        >
                            Agregar
                        </button>
    
                        <button
                            type="button"
                            class="btn btn-primary"
                            on:click|preventDefault={actualizarEmpresa}
                            
                        >
                            Actualizar
                        </button>
                    </form>
                </div>
            </div>
        <div style="width: 8%;"></div>
        <div class="card border-dark" style="width: 50%; ">
            <table class="table" >
                <thead>
                    <tr>
                        <th>ID</th>
                        <th>Cliente</th>
                        <th>Fecha Registro </th>
                        <th>Fecha Expiración</th>
                        <th></th>
                    </tr>
                </thead>
    
                <tbody>
                    {#each res as res}
                        <tr>
                            <td>{res.MEMID}</td>
                            <td>{res.NOMBRECLI}</td>
                            <td>{res.FECHAREGMEM}</td>
                            <td>{res.FECHAEXOMEM}</td>
                            <td>
                                <button
                                    type="submit"
                                    class="btn btn-primary"
                                    on:click={editarEmpresa(res)}
                                    >Editar
                                </button>
        
                                <button
                                    type="submit"
                                    class="btn btn-danger"
                                    on:click={borrarEmpresa(res.MEMID)}
                                >
                                    Borrar</button
                                >
                            </td>
                        </tr>
                    {/each}
                </tbody>
            </table>
        </div>
    </div>
</body>




<style>
    .pater > div {
   display: inline-block;
   width: 24%;
    /*** Sólo a efectos de visualización ***/
   margin: 0;
    }
   .card{
    display: flex;
  justify-content: center;
  align-items: center;
  background-color: beige;
 }
button{
	color: white;
	height: 40px;
	cursor: pointer;
	margin-top: 5px;
	font-size: 0.8em;
	font-family: 'Sniglet', cursive;
	-webkit-transition: background-color 0.5s;
	-moz-transition: background-color 0.5s;
	-o-transition: background-color 0.5s;
	transition: background-color 0.5s;
}
body{
    font-family:Arial, Helvetica, sans-serif, cursive;
	font-size: 1.15em;
    background: #076585;  /* fallback for old browsers */
background: -webkit-linear-gradient(to right, #fff, #076585);  /* Chrome 10-25, Safari 5.1-6 */
background: linear-gradient(to right, #fff, #076585); /* W3C, IE 10+/ Edge, Firefox 16+, Chrome 26+, Opera 12+, Safari 7+ */


}
.card-header{
    font-size: 1.2em;
    background: #083f5f;  
color: azure;
}
.form-label{
    font-weight: bold;
}
th{
    background: #00a2ec;  
}
tr{
    border-radius: 5px;
}
</style>

